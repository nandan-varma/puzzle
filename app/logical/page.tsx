...
export default function Logical() {
  ...
  const [selected, setSelected] = useState<{ poleIndex: number; value: number } | null>(null);
  ...
  const handleClick = (poleIndex: number) => {
    setPoles((prevPoles) => {
      ...
      if (selected === null) {
        ...
        if (clickedPole.length === 0 || (topDisk !== undefined && selected.value < topDisk)) {
  ...
  return (
    <Container>
      ...
      {selected && (
        <p style={{ marginBottom: '1rem', color: '#8b5cf6', fontWeight: 'bold' }}>
            ...
            {pole.map((v, j) => (
              <div
                ...
                style={{
                  ...
                  backgroundColor: selected?.value === v ? '#6366f1' : '#8b5cf6',
                  borderRadius: '4px',
                  margin: '2px auto',
                  width: `${v * 20}%`,
                  minWidth: '20px',
